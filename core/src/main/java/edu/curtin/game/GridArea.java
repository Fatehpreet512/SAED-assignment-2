...
/**
 * A JavaFX GUI element that implements an automatically-scaling on-screen area for displaying
 * a 2D grid-based game world with captioned icons.
 * 
 * This class is part of the assignment 1 starter code, adapted for the maze game.
 * It provides a canvas-based display system that can show:
 * - A 2D grid with configurable dimensions
 * - Icons representing game objects (player, items, obstacles, goals)
 * - Text captions for each icon
 * - Automatic scaling to fit the available space
 * - Grid lines for visual clarity
 * 
 * Usage in the maze game:
 * - Grid dimensions represent the game world size
 * - Icons represent player (P), goal (G), items (I), obstacles (X)
 * - Text captions provide additional information
 * - The display updates automatically when game state changes
 * 
 * Key methods:
 * - addIcon(): Add new icons to the display
 * - requestLayout(): Trigger a redraw after changes
 * - drawIcon(): Render individual icons with text
 * 
 * This class handles the visual representation of the game grid,
 * while GameState manages the logical game data.
 */
public class GridArea extends Pane
{
    private double gridWidth;              // Width of the grid in grid units
    private double gridHeight;             // Height of the grid in grid units
    private double gridSquareSize = 1.0;  // Size of each grid square (recalculated for scaling)
    private boolean gridLines = true;      // Whether to draw grid lines
    private Color captionColour = Color.WHITE; // Color for text captions
    private List<GridAreaIcon> icons = new ArrayList<>(); // List of icons to display
    private Canvas canvas = null;          // Canvas for drawing the grid and icons

    /**
     * Create a new GridArea with the specified dimensions.
     * 
    ...
    private void drawIcon(GraphicsContext gfx, GridAreaIcon icon)
    {
        ...
        double fullSizePixelWidth, fullSizePixelHeight;
        
        if (image != null) {
            fullSizePixelWidth = image.getWidth();
            fullSizePixelHeight = image.getHeight();
        } else {
            // Default size for text display
            fullSizePixelWidth = 50;
            fullSizePixelHeight = 50;
        }
        ...
        if (image != null) {
            ...
            // Draw text instead of image
            gfx.setFill(Color.WHITE);
            gfx.setStroke(Color.BLACK);
            gfx.setFont(new javafx.scene.text.Font(24));
            gfx.setTextAlign(TextAlignment.CENTER);
            gfx.setTextBaseline(VPos.CENTER);
            gfx.strokeText(icon.getCaption(), 0, 0);
            gfx.fillText(icon.getCaption(), 0, 0);
        }
        gfx.restore();
